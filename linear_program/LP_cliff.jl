"""
Calculate EVaR values computed by linear program on cliff walking(CW) domain
Risk level α = 0.2, this is for figure 5
"""
...
function main_evar()
    ...
    # risk level of EVaR for cliff walking domain 
    alpha_array = [0.2]
    
    #calculate the EVaR value and return it
    evars = compute_optimal_policy(alpha_array, initial_state_pro, model,δ)
    
    println("EVaR risk level is 0.2 on CW domain, EVaR value is  ", evars[1])

...
# Compute EVaR values for different number of samples on cliffwalking domain. 
# The EVaR risk level α is 0.2
# The EVaR values are used in plot_lp_q_cliff.jl
...
function compute_B(α,δ,beta0)
   ...
   maxBetaValue = 6
    ...
    while beta < βk && beta <  maxBetaValue
        beta_next = beta * c /(c-beta*δ) *1.05
        if beta_next < βk
            push!(B,beta_next)
        end
        beta = beta_next
...
for β in B
    ...
    # Numbe of samples =  [500,1000,2000,3000,4000,5000,6000,8000,10000,20000,30000]
    n_steps = 30000
    ...
    lr = Counter(1e-10,harmonicDecay,decay_rate=1e-4,ϵ = 0.0012)
    sa_method = "erm"
...
println("The EVaR risk level α is 0.2 on CW domain. The EVaR value is  : ", evar_value)

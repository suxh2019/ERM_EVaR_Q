# Using algorithm 2 in the paper to compute EVaR values for different number of samples
# on gambler ruin(GR) domain
# The EVaR values are used to plot figure 6 in the paper
...
#-------------------------
# q-learning on domain gambler ruin(GR) domain, EVaR risk level α = 0.2
# Compute the EVaR value and the optimal policy using Q-learning algorithm
#-----------------------------------
d= "gambler.jld2"
domain = d[1:end-5]
...
function compute_B(α,δ,beta0)
    ...
    while beta < maxBetaValue &&  beta < βk
...
u_s0 = ones(mdp.lSl) ./ (mdp.lSl-2)
u_s0[end] = 0 # Sink state: reach the cap 7
u_s0[1] =0 # sink state: lose all money
...
for β in B
    ...
    # n_steps represents the number of samples
    #Manually change the number of samples
    # Number of samples = [20, 200,1000,3000,5000,10000,15000,20000,30000]
    n_steps = 10000
    ...
    lr = Counter(1e-10,harmonicDecay,decay_rate=1e-4,ϵ = 0.003)
    sa_method = "erm"
...
println(" EVaR risk level α = 0.2 on GR domain, the EVaR value is : ", evar_value)

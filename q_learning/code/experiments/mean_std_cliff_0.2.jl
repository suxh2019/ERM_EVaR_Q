...
# and calculate its EVaR value on cliff walking(CW) domain
# the optimal policy is for Figure 4
# EVaR risk level α = 0.2

#Manually change the random seed and the number of samples and record the EVaR values used for figure 4
#EVaR values are used in mean_std.jl under directory linear_program
...
for β in B
    ...
    #---------------------------------------------------
    # n_steps represent the number of samples, change the number of samples
    # number of samples = [500,1000,2000,3000,4000,5000,6000,8000,10000,20000,30000]
    n_steps = 30000
    #---------------------------------------------------
    ...
    #---------------------------------------------------
    # Change the random seed and generate different samples
    # random seeds are 0,100, 444, 5000,10000,22222
    seed = 0
    #---------------------------------------------------
    ...
    lr = Counter(1e-10,harmonicDecay,decay_rate=1e-4,ϵ = 0.0017)
    sa_method = "erm"
...
println("Risk level α = 0.2 on CW domain, EVaR value is : ", evar_value)
